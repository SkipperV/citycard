...
export default function Edit({auth, status, city, transportRoute}) {
    ...
    const [routeNumber, setRouteNumber] = useState(transportRoute.route_number);
    const [transportType, setTransportType] = useState(transportRoute.transport_type);
    const [endpoint_1, setEndpoint_1] = useState(transportRoute.route_endpoint_1);
    const [endpoint_2, setEndpoint_2] = useState(transportRoute.route_endpoint_2);
    const [errors, setErrors] = useState({});

    const mutation = useMutation({
        mutationFn: async ({routeNumber, transportType, endpoint_1, endpoint_2}) => {
            return await axios.put(route('api.transport.update', {city: city.id, transportRoute: transportRoute.id}), {
